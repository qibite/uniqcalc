jQuery(document).ready(($)=>{
	...
	$('#cicO').click(()=>{
		...
		var data_cran = { action: 'calc_cran', _gp:cran.gp, _shir:cran.paramsO.shpO, _upravl:cran._3 }
			$.post( calc_ajaxurl.url, data_cran, function(response) {
				...
				$('#obshprm').siblings('strong').text(String(Number(price_crane).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
				$('#vzrivoshow').siblings('strong').text(String(Number(vzr).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
				$('#pozharshow').siblings('strong').text(String(Number(pozh).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
		});
	});
		$('#obshprm').click(()=>{
			cran.setszo = 'IP44';
	...
	$('#razmeshO').click(()=>{
		...
		$('#Навес > strong.price_').text(String(Number(summ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 '));
		$('#Помещение > strong.price_').text(String(Number(summ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 '));
	...
	$('#type_uprO').click(()=>{showChange($('#тип_управления'))})
		$('#Ручное').click(()=>{
			...
			$('#option_2 > .chastotnie_preobrazovateli').remove();
			$('#option_2 > .sirena').remove();
			$('#option_2 > .opticheskiy_datchik').remove();
			$('#option_2 > .registrator').remove();
			$('#option_2 > .ogranicitel').remove();
			$('#option_2 > .vesi').remove();
			$('#option_2 > .chastotniyprivodtali').remove();
			$('#option_2 > .uzob').remove();
			$('#option_2 > .telezhka').remove();
			$('#option_2 > .chetirekolesa').remove();
			$('#option_2 > .stop').remove();
			$('#option_2 > .tormoz').remove();
			$('#option_2 > .shit_electro').remove();
			$('#option_2 > .vikluchateli').remove();
			$('#option_2 > .vikluchatelioptik').remove();
		...
		$('#Электро').click(()=>{
			...
			$('#Ospeed').text(cran.speedmetr);
			cran.setszo = cran._2.search(/Пожаробезопасное/i) != -1 ? 'IP54' : cran._2.search(/Взрывобезопасное/i) != -1 ? 'IP65' : 'IP44';
			$('#Oszo').text(cran.szo);
	...
	$('#cOszo').click(()=>{showChange($('#степень_защиты'))});
		...
		$('#IP44').click(()=>{
			if(cran._2.search(/Пожаробезопасное/i) != -1){alert('При пожаробезопасном исполнении крана этот вариант защиты невозможен!'); return};
			if(cran._2.search(/Взрывобезопасное/i) != -1){alert('При взрывобезопасном исполнении крана этот вариант защиты невозможен!'); return};
			cran.setszo = 'IP44';
			setTimeout(()=>{hideChange()},300);
			$('#Oszo').text(cran.szo);
			cran.calculate_oporniy_crane()
		});
		$('#IP54').click(()=>{if(cran._2.search(/Взрывобезопасное/i) != -1){alert('При взрывобезопасном исполнении крана этот вариант защиты невозможен!'); return};cran.setszo = 'IP54';setTimeout(()=>{hideChange()},300);$('#Oszo').text(cran.szo);cran.calculate_oporniy_crane()});
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$('#cicP').click(()=>{
		...
		var data_cran_p = { action: 'calc_cran_p', _gp:cran.gp, _shir:cran.paramsP.shpP, _upravl:cran._3 }
			$.post( calc_ajaxurl.url, data_cran_p, function(response) {
				...
				let vzr = price_crane + (price_crane*35/100);
				let pozh = price_crane + (price_crane*10/100);
				$('#obshprm2').siblings('strong').text(String(Number(price_crane).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
				$('#vzrivoshow2').siblings('strong').text(String(Number(vzr).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
				$('#pozharshow2').siblings('strong').text(String(Number(pozh).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽');
			})
	})
	...
	$('#razmeshP').click(()=>{
		...
		$('#НавесP > strong.price_').text(String(Number(summ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 '));
		$('#ПомещениеP > strong.price_').text(String(Number(summ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 '));
		summ = Number(summ) + 9000;
	...
	$('#cPszo').click(()=>{showChange($('#степень_защитыP'))});
		...
		$('#IP54P').click(()=>{if(cran._2.search(/Взрывобезопасное/i) != -1){alert('При взрывобезопасном исполнении крана этот вариант защиты невозможен!'); return};cran.setszo = 'IP54';setTimeout(()=>{hideChange()},300);$('#Pszo').text(cran.szo);cran.calculate_podvesnoy_crane()});
...
*******************************************************************************************************************************************************************************************************************************************************************/
	function cl_chastotniki_crana () {
		$.post( calc_ajaxurl.url, { action: 'chastotniki_insert', _gp:cran.gp, _vbi:cran._2.search(/Взрывобезопасное/i) != -1?1:0 }, function(response)
		{
			...
			$('#b8_2_5 > .hiddened').text(String(Number(chst_resp._85).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#b8_2_6 > .hiddened').text(String(Number(chst_resp._86).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#b8_2_7 > .hiddened').text(String(Number(chst_resp._87).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			console.log(String(Number(chst_resp._85).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 '))
			if (cran._2.search(/Взрывобезопасное/i) != -1 && cran.gp != 500) {
	...
	function calculate_tal (argument) {
			//////////////
				...
				$.post( calc_ajaxurl.url, data_tal, function(response) {
					...
					+ tal.country +'</h4> \
					<p> \
						<span class="stoimost_parametra">'+ String(Number(tal.summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽</span> \
					</p>');
				});
			//////////////
	}
	...
	function calculate_shit () {
			//////////////
				...
				$.post( calc_ajaxurl.url, data_shit, function(response) {
					$('#b11 > .hiddened').text(String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
			//////////////
	}
			...
			let stop =
				...
				var data_stop = { action: 'calc_stop', _stop_id:stop.id }
					$.post( calc_ajaxurl.url, data_stop, function(response) {
						$('#b13 .hiddened').text(String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					});
				/////////////
		}
	}
	...
	function calculate_tormoz () {
		if (cran._3 != 'Ручное') {
				...
				$.post( calc_ajaxurl.url, data_tormoz, function(response) {
					$('#b14 > .hiddened').text(String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
	...
	function calculate_pokraska () {
			...
			var data_pokraska = { action: 'calc_pokraska', _pokraska_id:pokraska.id }
				$.post( calc_ajaxurl.url, data_pokraska, function(response) {
					$('#b18 > .hiddened').text(String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
			/////////////
	}
	...
	function calculate_chstotniyPrivodTali () {
			...
			var data_chpt = { action: 'calc_chstotniy_privod_tali', _gp:cran.gp }
				$.post( calc_ajaxurl.url, data_chpt, function(response) {
					...
					$('#b28_1 > .hiddened').text(String(Number(chpt.ESQ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#b28_2 > .hiddened').text(String(Number(chpt.HYUNDAI).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#b28_3 > .hiddened').text(String(Number(chpt.DANFOSS).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
			/////////////
	}
	...
	function calculate_ogranichitel () {
			...
			var data_chpt = { action: 'calc_ogranichitel', _gp:cran.gp }
				$.post( calc_ajaxurl.url, data_chpt, function(response) {
					...
					$('#b21_1 > .hiddened').text(String(Number(ogr.ctifrovoy).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#b21_2 > .hiddened').text(String(Number(ogr.kanatniy).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
			/////////////
	}
	...
	function calculate_vesi () {
			...
			var data_chpt = { action: 'calculate_vesi', _gp:cran.gp }
				$.post( calc_ajaxurl.url, data_chpt, function(response) {
					...
					$('#b22_1 > .hiddened').text(String(Number(vesi.russia).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#b22_2 > .hiddened').text(String(Number(vesi.korea).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
			/////////////
	}
/*******************************************************************************************************************************************************************************************************************************************************************
...
function postavka_tokopodvoda () {
	...
	var data_all_tok = { action: 'tokopodvod_all',  _dlinna:cran._1 == 'Опорный' ? cran.paramsO.dpO : cran.paramsP.dpP  };
		$.post( calc_ajaxurl.url, data_all_tok, function (response) {
			...
			$('#tokoprovod1 > span.hiddened').text(String(Number(result.tok1).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#tokoprovod2 > span.hiddened').text(String(Number(result.tok2).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#tokoprovod3 > span.hiddened').text(String(Number(result.tok3).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#tokoprovod4 > span.hiddened').text(String(Number(result.tok4).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('#tokoprovod5 > span.hiddened').text(String(Number(result.tok5).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		})
	/////////////
}
...
/*$('#a3').on('click', '.tokoprovod', function(event) {
	...
	stoim = stoim[0];
	{
		...
		$(this).children('.hiddened').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
	}
});	*/
		...
		let prompt_city = document.createElement('div');
			...
			savecity.addEventListener('click', function savci () {
				...
				$.post( calc_ajaxurl.url, data_distance, function(response)
				{
					...
					$('#c1 .hiddened').text(String(Number(result.price_montazh).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#c2 .opisanie_parametra').html('в пункте '+cran.city+'<br>включая проживание');
					$('#c2 .hiddened').text(String(Number(result.price_shef_montazh).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#c3 .opisanie_parametra').html('до пункта '+cran.city+'<br>Расстояние - '+result.distance+' км');
					$('#c3 .hiddened').text(String(Number(result.dostavka_price).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('#c6 .opisanie_parametra').html('до пункта '+cran.city+'<br>Расстояние - '+result.distance+' км');
					$('#c6 .hiddened').text(String(Number(result.price_expert).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				});
				prompt_city.remove();
			})
	}
	...
	var data_all_tok = { action: 'tokopodvod_montazh',  _dlinna:cran._1 == 'Опорный' ? cran.paramsO.dpO : cran.paramsP.dpP  };
		$.post( calc_ajaxurl.url, data_all_tok, function (response) {
			...
			if ($('#option_1 div.tokoprovod > h4').text() == 'Кабельный с кольцами') {
				...
				$('#c5 > .hiddened').text(String(Number(result.tok1).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			}
				...
				$('#c5 > .hiddened').text(String(Number(result.tok2).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			}
				...
				$('#c5 > .hiddened').text(String(Number(result.tok3).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			}
				...
				$('#c5 > .hiddened').text(String(Number(result.tok4).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			}
				...
				$('#c5 > .hiddened').text(String(Number(result.tok5).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			}
			else {
				$('#c5').css('display', 'none');
			}
	...
	for (let i=1; i<=10; i++) {
		...
		$.post( calc_ajaxurl.url, data_cran, function(response) {
			...
			if (cran._1 == 'Опорный') {
				switch (i) {
					case 1:
						$('.кг500_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 2:
						$('.кг1000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 3:
						$('.кг2000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 4:
						$('.кг3200_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 5:
						$('.кг5000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 6:
						$('.кг6300_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 7:
						$('.кг8000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 8:
						$('.кг10000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 9:
						$('.кг12500_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 10:
						$('.кг16000_resume_o > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					default:
						gp=false;
						break;
				...
				switch (i) {
					case 1:
						$('.кг500_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 2:
						$('.кг1000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 3:
						$('.кг2000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 4:
						$('.кг3200_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 5:
						$('.кг5000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 6:
						$('.кг6300_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 7:
						$('.кг8000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 8:
						$('.кг10000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 9:
						$('.кг12500_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					case 10:
						$('.кг16000_resume_p > strong.price_').text(String(Number(summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						break;
					default:
						gp=false;
						break;
