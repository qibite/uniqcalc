...
************************************************************************************************************************/
	...
	$('button._37, img._37, button._37_j, img._37_j, button._37vbi, img._37vbi').click(function () {$('#main_block_for_steps').css('height', '1000px');
		...
		hide($('#_3'));
		cran.step = 'Шаг 4 из 11'
		cran._3Type = cran._2.search(/Взрывобезопасное/i) != -1 ? cran._3Type + ' Telecrane (Тайвань) ВБИ':cran._3Type + ' Telecrane (Тайвань)';
		next_group( $('#грузоподъемность'), 0 ); // НОВОЕ
		//next_group( $('#razmeshenie'), 0 ); СТАРОЕ
	});
	$('button._38, img._38, button._38_j, img._38_j, button._38vbi, img._38vbi').click(function () {$('#main_block_for_steps').css('height', '1000px');
		...
		hide($('#_3'));
		cran.step = 'Шаг 4 из 11'
		cran._3Type = cran._2.search(/Взрывобезопасное/i) != -1 ? ' cran._3Type + Ikusi (Испания) ВБИ':cran._3Type + ' Ikusi (Испания)';
		next_group( $('#грузоподъемность'), 0 ); // НОВОЕ
		//next_group( $('#razmeshenie'), 0 ); СТАРОЕ
	});
	$('button._39, img._39, button._39_j, img._39_j, button._39vbi, img._39vbi').click(function () {$('#main_block_for_steps').css('height', '1000px');
		...
		hide($('#_3'));
		cran.step = 'Шаг 4 из 11'
		cran._3Type = cran._2.search(/Взрывобезопасное/i) != -1 ? cran._3Type + ' HBC-Radiomatic (Германия) ВБИ':cran._3Type + ' HBC-Radiomatic (Германия)';
		next_group( $('#грузоподъемность'), 0 ); // НОВОЕ
		//next_group( $('#razmeshenie'), 0 ); СТАРОЕ
	});
/************************************************************************************************************************
	...
	$('.tal_yes').click(()=>{
		...
		$('.electro_t').click(()=>{
			...
			if (cran._2.search(/Взрывобезопасное/i) == 0) {
				...
				tal.img = '17.5.png';
				$('.stdV_t').siblings('.price_').text(String(Number(tal.priceAJAX.bolgariya_el_canatVBI_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				$('.umenV_t').siblings('.price_').text(String(Number(tal.priceAJAX.bolgariya_el_canatUSVVBI_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			...
			$('.bolgariya_tE').click(()=>{
				...
				if (tal.gp <= 2000 && tal.visota <= 12000) {
					...
					$('.kanat_t').siblings('.price_').text('От ' + String(Number(tal.priceAJAX.bolgariya_el_canat_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					$('.cep_t').siblings('.price_').text(String(Number(tal.priceAJAX.bolgariya_el_cep_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					easyscroll(1300);
				} else {
					$('.hide_s_visota_tal').removeClass('hide_s_visota_tal').addClass('show_s_visota_tal');
					$('#main_block_for_steps').css('height', '2386px');
					...
					if (cran._2.search(/Взрывобезопасное/i) == 0) {
						...
						$('.stdV_t').siblings('.price_').text(String(Number(tal.priceAJAX.bolgariya_el_canat_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
						$('.umenV_t').siblings('.price_').text(String(Number(tal.priceAJAX.bolgariya_el_canatUSV_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
					}
					$('.show_type_tal').removeClass('show_type_tal').addClass('hide_type_tal');
				  }
				tal.country = 'Болгария';
				...
				$('.stdV_t').click(()=>{
					...
					if (cran._2.search(/Взрывобезопасное/i) != -1 && tal.country == 'Болгария') {$('#main_block_for_steps').css('height', '1930px');}
			...
			$('.china_tE').click(()=>{
				...
				$('.kanat_t').siblings('.price_').text(String(Number(tal.priceAJAX.china_el_canat_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				$('.cep_t').siblings('.price_').text(String(Number(tal.priceAJAX.china_el_cep_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
				$('h4.bolgariya_tE').css({'background-color':'#cecece', 'color':'#242526'});
				$('h4.russia_tE').css({'background-color':'#cecece', 'color':'#242526'});
			...
			if (price.id == 0) return false;
				...
				$.post( calc_ajaxurl.url, data_crane_electro_upravleniya, function (response) {
					...
					$('#option_2 .dop_parametr:last-child').before('<div class="dop_parametr"><span class="change_this_option_with_del pult_change"><i class="fa fa-pencil-square" aria-hidden="true"></i> Изменить</span><span class="del_this_option return_pult_default"><i class="fa fa-trash-o" aria-hidden="true"></i></span><img src=" \
						...
						<span class="stoimost_parametra">'+ String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽</span><i class="id_bro">b3</i></p>');
					calc_all_dop();
				});
			//////////////
}
...
cran.chastotnik_price = function (model) {
	...
	var data_chastotnik = { action: 'calc_chastotnik', _chastotnik_id:price.id, _gp:cran.gp, _vbi:cran._2.search(/Взрывобезопасное/i) != -1?1:0 }
	$.post( calc_ajaxurl.url, data_chastotnik, function (response) {
		$('#option_2 .dop_parametr:last-child').before('<div class="dop_parametr"><span class="change_this_option_with_del preobrazovatel_change"><i class="fa fa-pencil-square" aria-hidden="true"></i> Изменить</span><span class="del_this_option return_preobrazovatel_default"><i class="fa fa-trash-o" aria-hidden="true"></i></span><img src=" \
			...
			<span class="stoimost_parametra">'+ String(Number(response).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽</span></p>');
		calc_all_dop();
	});
}
			...
			default:
				$.post( calc_ajaxurl.url, data_motor, function (response) {
					...
					$('#option_2 .dop_parametr:last-child').before('<div class="dop_parametr"><span class="change_this_option_with_del motor_reductor_change"><i class="fa fa-pencil-square" aria-hidden="true"></i> Изменить</span><span class="del_this_option return_motor_default"><i class="fa fa-trash-o" aria-hidden="true"></i></span><img src=" \
						...
						<span class="stoimost_parametra">'+ String(Number(answer.base_price).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽</span></p>');
						$('#option_click2').removeClass('head_dop_menu').addClass('head_dop_menu_open');
						$('#option_2').removeClass('dop_menu').addClass('dop_menu_open');
					calc_all_dop();
				})
...
cran.calculate_oporniy_crane = function  () {
				...
				$.post( calc_ajaxurl.url, data_cran, function(response) {
					...
					if (cran._2.search(/Пожаробезопасное/i) != -1) {
						$('#Oic').parent().siblings('.doplata').text(String(Number((price_crane*10)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran._2.search(/Взрывобезопасное/i) != -1) {
						$('#Oic').parent().siblings('.doplata').text(String(Number((price_crane*35)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					...
					// ТЕМПЕРАТУРА
					if (cran.temper[0] == '-50') {
						$('#Otr').parent().siblings('.doplataTEMPER').children('#tminsO').text(String(Number((price_crane*25)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran.temper[0] == '-40') {
						$('#Otr').parent().siblings('.doplataTEMPER').children('#tminsO').text(String(Number((price_crane*10)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran.temper[0] == '-20') {
					...
					if (cran.temper[1] == '+60') {
						$('#Otr').parent().siblings('.doplataTEMPER').children('#tplusO').text(String(Number((price_crane*15)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					...
					if (cran.szo == 'IP54') {
						$('#Oszo').parent().siblings('.doplata').text(String(Number((price_crane*8)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Oszo').text(cran.szo);
					}
					if (cran.szo == 'IP65') {
						$('#Oszo').parent().siblings('.doplata').text(String(Number((price_crane*30)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Oszo').text(cran.szo);
					}
					if (cran.szo == 'IP65' && cran._2.search(/Пожаробезопасное/i) != -1) {
						$('#Oszo').parent().siblings('.doplata').text(String(Number((price_crane*25)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Oszo').text(cran.szo);
					}
					if (cran.szo == 'IP65' && cran._2.search(/Взрывобезопасное/i) != -1) {
						$('#Oszo').parent().siblings('.doplata').text(String(Number((price_crane*30)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Oszo').text(cran.szo);
					}

					{
						if (cran.paramsO.shpO > 28500) {
							...
							$('#summa').parent().html('Базовая стоимость крана: <span id="summa">'+String(Number(cran.summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+'</span> ₽');
						}
					}
				});
...
cran.calculate_podvesnoy_crane = function  () {
				...
				$.post( calc_ajaxurl.url, data_cran_p, function(response) {
					...
					// РАСПОЛОЖЕНИЕ
					if (cran._2.search(/Пожаробезопасное/i) != -1) {
						$('#Pic').parent().siblings('.doplata').text(String(Number((price_crane*10)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran._2.search(/Взрывобезопасное/i) != -1) {
						$('#Pic').parent().siblings('.doplata').text(String(Number((price_crane*35)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran._2.search(/Общепромышленное/i) != -1) {
					...
					// ТЕМПЕРАТУРА
					if (cran.temper[0] == '-50') {
						$('#Ptr').parent().siblings('.doplataTEMPER').children('#tminsP').text(String(Number((price_crane*25)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran.temper[0] == '-40') {
						$('#Ptr').parent().siblings('.doplataTEMPER').children('#tminsP').text(String(Number((price_crane*10)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}
					if (cran.temper[0] == '-20') {
					...
					if (cran.temper[1] == '+60') {
						$('#Ptr').parent().siblings('.doplataTEMPER').children('#tplusP').text(String(Number((price_crane*15)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					}

					//
					{
						if (cran.paramsP.shpP <= 6000 && cran.paramsP.vpP >= 900) {
							$('#Pdk').parent().siblings('.doplata').text(String(Number((price_crane*15)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						}
						else {
					...
					if (cran.szo == 'IP54') {
						$('#Pszo').parent().siblings('.doplata').text(String(Number((price_crane*8)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Pszo').text(cran.szo);
					}
					if (cran.szo == 'IP65') {
						$('#Pszo').parent().siblings('.doplata').text(String(Number((price_crane*30)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Pszo').text(cran.szo);
					}
					if (cran.szo == 'IP65' && cran._2.search(/Пожаробезопасное/i) != -1) {
						$('#Pszo').parent().siblings('.doplata').text(String(Number((price_crane*25)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
						$('#Pszo').text(cran.szo);
					}
					if (cran.szo == 'IP65' && cran._2.search(/Взрывобезопасное/i) != -1) {
						$('#Pszo').parent().siblings('.doplata').text(String(Number((price_crane*30)/100).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+' ₽');
					...
					}
					{
						if (cran.paramsP.shpP > 15000) {
							...
							$('#summa_p').parent().html('Базовая стоимость крана: <span id="summa_p">'+String(Number(cran.summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')+'</span> ₽');
						}
					}
	...
	$.post( calc_ajaxurl.url, data_tal, function(response) {
		...
		$('#option_2 .dop_parametr:last-child').before('<div class="dop_parametr tal_for_search tali"><span class="change_this_option_with_del tal_remake"><i class="fa fa-pencil-square" aria-hidden="true"></i> Изменить</span> \
			...
			<p><span class="opisanie_parametra">'+String(tal.upravlenie)+'<br>'+String(tal.type)+iii+'</span><br> \
				<span class="stoimost_parametra">'+ String(Number(tal.summa).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') +' ₽</span><i class="id_bro">b17</i> \
			</p>');
	});
...
function gp_insert_price ()
{
	...
	$.post( calc_ajaxurl.url, gp_data, function(response) {
		...
		if (cran.step != 'end') {
			$('.кг500 > span > .price_').text('От ' + String(Number(gp_resp._500).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг1000 > span > .price_').text('От ' + String(Number(gp_resp._1000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг2000 > span > .price_').text('От ' + String(Number(gp_resp._2000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг3200 > span > .price_').text('От ' + String(Number(gp_resp._3200).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг5000 > span > .price_').text('От ' + String(Number(gp_resp._5000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг6300 > span > .price_').text('От ' + String(Number(gp_resp._6300).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг8000 > span > .price_').text('От ' + String(Number(gp_resp._8000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг10000 > span > .price_').text('От ' + String(Number(gp_resp._10000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг12500 > span > .price_').text('От ' + String(Number(gp_resp._12500).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
			$('.кг16000 > span > .price_').text('От ' + String(Number(gp_resp._16000).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		}
	});
}
...
{
	$.post( calc_ajaxurl.url, { action: 'chastotniki_insert', _gp:cran.gp, _vbi:cran._2.search(/Взрывобезопасное/i) != -1?1:0 }, function(response)
	{
		let chst_resp = JSON.parse(response);
		$('.esq').siblings('.price_').text(String(Number(chst_resp._85).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.hyundai').siblings('.price_').text(String(Number(chst_resp._86).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.danfross').siblings('.price_').text(String(Number(chst_resp._87).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.несколько_с').siblings('.price_').text('От ' + String(Number(chst_resp.dbls).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
	});
}
...
function motor_insert_price ()
{
	$.post( calc_ajaxurl.url, { action: 'motor_insert', _gp:cran.gp, _shir:cran._1 == 'Опорный' ? cran.paramsO.shpO : cran.paramsP.shpP }, function(response)
	{
		...
		$('.чнг').siblings('.price_').text(String(Number(motor_resp._92).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.цбб').siblings('.price_').text(String(Number(motor_resp._93).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.цнг').siblings('.price_').text(String(Number(motor_resp._94).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.цер').siblings('.price_').text(String(Number(motor_resp._95).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
	});
}
...
function tali_insert_price ()
{
	$.post( calc_ajaxurl.url, { action: 'tal_insert', _gp:cran.gp, _upravlenie:tal.upravlenie, _visota:tal.visota, _ispolnenie:tal.ispolnnie, _cranisp:cran._2, temper_minus:cran.temper[0], temper_plus:cran.temper[1] }, function(response)
	{
		...
		$('.russia_tR').siblings('.price_').text(String(Number(tal.priceAJAX.rt_r).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.china_tR').siblings('.price_').text(String(Number(tal.priceAJAX.ct_r).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.sheciya_tR').siblings('.price_').text(String(Number(tal.priceAJAX.st_r).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.bolgariya_tE').siblings('.price_').text('От ' + String(Number(tal.priceAJAX.bolgariya_el_cep_t == null? tal.priceAJAX.bolgariya_el_canat_t : tal.priceAJAX.bolgariya_el_cep_t < tal.priceAJAX.bolgariya_el_canat_t ? tal.priceAJAX.bolgariya_el_cep_t : tal.priceAJAX.bolgariya_el_canat_t ).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.russia_tE').siblings('.price_').text(String(Number(tal.priceAJAX.russia_el_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
		$('.china_tE').siblings('.price_').text('От ' + String(Number(tal.priceAJAX.china_el_canat_t).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ₽');
	});
}
...
""))),b+=parseInt(a.innerText.replace(/\s/g,"")))}),document.getElementById("doplata").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 "),document.getElementById("summa4").innerText="\u041f\u043e\u0434\u043e\u0436\u0434\u0438\u0442\u0435...",setTimeout(function(){calc_all_oporniy()},1200)):(document.querySelectorAll("#\u0440\u0435\u0437\u0443\u043b\u044c\u0442\u0430\u0442_\u043f\u043e\u0434\u0432\u0435\u0441\u043d\u043e\u0433\u043e > .res tbody .doplata, #\u0440\u0435\u0437\u0443\u043b\u044c\u0442\u0430\u0442_\u043f\u043e\u0434\u0432\u0435\u0441\u043d\u043e\u0433\u043e > .res tbody .doplataTEMPER > span").forEach(function(a,
d){isNaN(parseInt(a.innerText.replace(/\s/g,"")))||(console.log(parseInt(a.innerText.replace(/\s/g,""))),b+=parseInt(a.innerText.replace(/\s/g,"")))}),document.getElementById("doplata_p").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 "),document.getElementById("summa4_p").innerText="\u041f\u043e\u0434\u043e\u0436\u0434\u0438\u0442\u0435...",setTimeout(function(){calc_all_podvesnoy()},1200))}
...
"")))});"\u041e\u043f\u043e\u0440\u043d\u044b\u0439"==cran._1?(document.getElementById("summa2").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 "),document.getElementById("summa4").innerText="\u041f\u043e\u0434\u043e\u0436\u0434\u0438\u0442\u0435..."):(document.getElementById("summa2_p").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 "),document.getElementById("summa4_p").innerText="\u041f\u043e\u0434\u043e\u0436\u0434\u0438\u0442\u0435...");
setTimeout(function(){"\u041e\u043f\u043e\u0440\u043d\u044b\u0439"==cran._1?calc_all_oporniy():calc_all_podvesnoy()},1200)}
function calc_all_uslugi(){var b=0;dop_parametri.uslugi=[];document.querySelectorAll("#option_3 > div.dop_parametr:not(:last-child)").forEach(function(a,d){isNaN(parseInt(a.children[3].children[2].innerText.replace(/\s/g,"")))||(b+=parseInt(a.children[3].children[2].innerText.replace(/\s/g,"")));dop_parametri.uslugi.push([a.children[2].innerText,a.children[3].children[2].innerText])});"\u041e\u043f\u043e\u0440\u043d\u044b\u0439"==cran._1?(document.getElementById("summa3").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,
"$1 "),document.getElementById("summa4").innerText="\u0421\u0447\u0438\u0442\u0430\u0435\u043c.."):(document.getElementById("summa3_p").innerText=String(Number(b).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 "),document.getElementById("summa4_p").innerText="\u0421\u0447\u0438\u0442\u0430\u0435\u043c..");setTimeout(function(){"\u041e\u043f\u043e\u0440\u043d\u044b\u0439"==cran._1?calc_all_oporniy():calc_all_podvesnoy()},1200)}
...
"")),c=parseInt(c.innerText.replace(/\s/g,"")),c=b+d+e+c):(b=parseInt(b.innerText.replace(/\s/g,"")),a=parseInt(a.innerText.replace(/\s/g,"")),d=parseInt(d.innerText.replace(/\s/g,"")),e=parseInt(e.innerText.replace(/\s/g,"")),c=parseInt(c.innerText.replace(/\s/g,"")),c=b+a+d+e+c);document.getElementById("summa4_p").innerText=String(Number(c).toFixed(0)).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1 ")}document.getElementById("next_tal").addEventListener("mouseover",check_dop_opcii_taley);
