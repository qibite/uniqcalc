...
<table class="tabla" style="width:49.5%;float:right">
<caption class="close_table"><span class="close">Открыть</span>Частотный привод перемещения тали</caption>
	<thead>
		<tr>
			<th>ID</th>
			<th>Производитель частотника</th>
			<th>Грузоподъемность</th>
			<th>Цена</th>
			<th class="ch_price">Редактировать цену</th>
		</tr>
	...
	<tbody>
		...
		$chastotnik_tali_result = $wpdb->get_results(
		"SELECT id, firma, gp, price FROM $chastotnik_tali");
		if ($chastotnik_tali_result) {
			foreach ($chastotnik_tali_result as $chastotnik) {
				$id = $chastotnik->id;
				$firma = $chastotnik->firma;
				$gp = $chastotnik->gp;;
				$price = $chastotnik->price;
				echo "<tr>
						<td class='id'>{$id}</td>
						<td>{$firma}</td>
						<td>{$gp} кг</td>
						<td class='price' style='position:relative'>
							...
							<div class='editing_price'>
								<form>
									<input type='hidden' name='db' value='$chastotnik_tali'>
									<input type='hidden' name='edited_id' value='$id'>
									<input type='number' name='new_price' value='$price' placeholder=''>
									<input type='button' name='price' value='Ок'>
								</form>
		...
		} else echo "<span style='font-size:1.3em;color:red'>База данных 'Частотный привод перемещения тали' не существует </span><a href='https://kwork.ru/user/qibite' target='blank'>Сообщить разработчику</a>";
	?>
	</tbody>
</table>

<table class="tabla" style="width:49.5%;float:left">
<caption class="close_table"><span class="close">Открыть</span>Надбавка на частотники ВБИ</caption>
	<thead>
		<tr>
			<th>ID</th>
			<th>Условие</th>
	...
	<tbody>
		...
		$chastotnik_nadbavka_result = $wpdb->get_results(
		"SELECT id, descript, price FROM $chastotnik_nadbavka");
		if ($chastotnik_nadbavka_result) {
			foreach ($chastotnik_nadbavka_result as $nadbavka) {
				$id = $nadbavka->id;
				$descript = $nadbavka->descript;
				$price = $nadbavka->price;
				echo "<tr>
						<td class='id'>{$id}</td>
						<td>{$descript}</td>
						<td class='price' style='position:relative'>
							...
							<div class='editing_price'>
								<form>
									<input type='hidden' name='db' value='$chastotnik_nadbavka'>
									<input type='hidden' name='edited_id' value='$id'>
									<input type='number' name='new_price' value='$price' placeholder=''>
									<input type='button' name='price' value='Ок'>
								</form>
		...
		} else echo "<span style='font-size:1.3em;color:red'>База данных 'Надбавка на частотники' не существует </span><a href='https://kwork.ru/user/qibite' target='blank'>Сообщить разработчику</a>";
	?>
	</tbody>
</table>
...
<caption class="close_table"><span class="close">Открыть</span>Варианты выбора пользовалеля (оборудование и опции) <!--<a href="<?php echo $url.'?page=unicalcplugin&view=delettable'; $_GET='variants';?>"> Удалить таблицу</a>--></caption>
	...
	<?php print_ops(24, 29, 'variants_vikl_result');?>

	<tr><td colspan="5" style="background-color:#fff429">12 Световая / Звуковая сирена</td></tr>
	<?php print_ops(29, 31, 'variants_svet_result');?>

	<tr><td colspan="5" style="background-color:#fff429">13 Аварийный СТОП с линейным выключателем</td></tr>
	<?php print_ops(31, 33, 'variants_stop_result');?>

	<tr><td colspan="5" style="background-color:#fff429">15 Регистраторы параметров</td></tr>
	<?php print_ops(33, 35, 'variants_tormoz_result');?>

	<tr><td colspan="5" style="background-color:#fff429">16 Устройства защиты</td></tr>
	<?php print_ops(35, 36, 'variants_tormoz_result');?>

	<tr><td colspan="5" style="background-color:#fff429">18 Индивидуальная покраска крана</td></tr>
	<?php print_ops(36, 39, 'variants_tormoz_result');?>

	<tr><td colspan="5" style="background-color:#fff429">21 Ограничитель грузоподъёмности</td></tr>
	<?php print_ops(39, 57, 'variants_stopgp_result');?>

	<tr><td colspan="5" style="background-color:#fff429">22 Крановые весы</td></tr>
	<?php print_ops(57, 77, 'variants_stopgp_result');?>

	<tr><td colspan="5" style="background-color:#fff429">Плавающая концевая тележка</td></tr>
	<?php print_ops(77, 78, 'variants_stopgp_result');?>

	<tr><td colspan="5" style="background-color:#fff429">Четыре приводных колеса</td></tr>
	<?php print_ops(78, 79, 'variants_stopgp_result');?>
	</tbody>
</table>
...
function print_ops($from, $to, $name_result)
{
	...
	} else {echo "<span style='font-size:1.3em;color:red'>База данных 'variants' не существует </span><a href='https://kwork.ru/user/qibite' target='blank'>Сообщить разработчику</a>";}
}
